...
namespace VIVE.OpenXR.Samples.Spectator.AdvDemo.Helper
{
    public static class GameObjectProcessHelper
    {
        ...
        {
            var components = Object.FindObjectsByType<T>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);

            if (components == null || components.Length == 0)
                return null;

            var result = new List<GameObject>(components.Length);
            for (int i = 0; i < components.Length; i++)
            {
                result.Add(components[i].gameObject);
            }
